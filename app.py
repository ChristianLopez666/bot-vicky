...
def notify_advisor(prospect_data, flow_type):
    ...
    if flow_type == "imss":
        ...
        message += f"• Monto solicitado: ${prospect_data.get('monto_solicitado', 'N/D')}\n"
...
def handle_imss_response(phone, message, user_flow):
    ...
    if step == "benefits_explanation":
        if "sí" in message.lower() or "si" in message.lower() or "quiero" in message.lower():
            ...
            user_flow["step"] = "check_requirements"
            user_flow["data"]["nomina_inbursa"] = False
            vx_wa_send_text(phone, "Entiendo perfectamente. La oferta seguirá disponible por si cambias de opinión.")
            vx_wa_send_text(phone, "Ahora verifiquemos que cumples con los requisitos...")
            vx_wa_send_text(phone, "¿Cuál es tu edad?")
        ...
        if "edad" not in data:
            ...
            try:
                ...
                if antiguedad >= 1:
                    ...
                    user_flow["step"] = "loan_amount"
                    vx_wa_send_text(phone, "¿Qué monto de préstamo deseas solicitar? (desde $40,000 hasta $650,000)")
                else:
    ...
    elif step == "loan_amount":
        try:
            monto = int(message.replace(',', '').replace('$', '').strip())
            data["monto_solicitado"] = monto
            
            if monto < 40000:
                vx_wa_send_text(phone, "❌ *Monto no disponible*\n\nDesafortunadamente no puedo apoyarte con montos menores a $40,000 MXN.\n\n💡 *Te sugiero considerar otras opciones:*")
                
                alternative_text = """¿Te interesa conocer otros productos disponibles?
...
3. Tarjetas VRIM"""
                ...
                vx_wa_send_text(phone, "El monto máximo para préstamos IMSS Ley 73 es de $650,000 MXN. Por favor ingresa un monto dentro del rango permitido:")
                
            else:
                # Monto válido, continuar con el proceso
                data["phone"] = phone
...
📞 Te contactaremos al número: {vx_last10(phone)}"""
            ...
            vx_wa_send_text(phone, "Por favor, ingresa un monto válido en números (ejemplo: 50000)")
