...
def extract_amounts(text):
    ...
    numbers = []
    # Busca patrones como 1,000, 1000, $1000, $1,000
    matches = re.findall(r'[\$]?[\d,]+\.?\d*', text)
    for match in matches:
        try:
            # Limpia caracteres no numéricos excepto punto decimal
...
def send_welcome_message(user_id):
    """Envía mensaje de bienvenida y pregunta por IMSS"""
    message = """¡Hola! Soy Vicky. ¿Estás interesado en préstamos para pensionados IMSS? (sí/no)"""
    
    send_whatsapp_message(user_id, message)
    
    # Actualizar estado
    session = get_user_session(user_id)
    session['state'] = 'ask_imss_interest'
    logger.info(f"Usuario {user_id} en estado: ask_imss_interest")

def handle_imss_interest(user_id, text):
    """Procesa respuesta sobre interés en IMSS"""
    session = get_user_session(user_id)
    text_lower = text.lower().strip()
    
    # Detectar sí
    positive_responses = ['sí', 'si', 'sip', 'yes', 'y', 'claro', 'acepto', 'ok', 'dale', 'por supuesto', 'afirmativo']
    negative_responses = ['no', 'nop', 'nope', 'negativo', 'na', 'non', 'para nada']
    
    if text_lower in positive_responses:
        # Iniciar flujo IMSS
        send_imss_initial_message(user_id)
        
    elif text_lower in negative_responses:
        # Preguntar por otro producto
        session['state'] = 'ask_product'
        message = "Entiendo. ¿Qué producto te interesa?"
        send_whatsapp_message(user_id, message)
        logger.info(f"Usuario {user_id} no interesado en IMSS, preguntando por otro producto")
        
    else:
        # Respuesta no reconocida
        message = "Por favor responde con sí o no. ¿Estás interesado en préstamos para pensionados IMSS?"
        send_whatsapp_message(user_id, message)

def handle_product_interest(user_id, text):
    """Procesa respuesta sobre otro producto de interés"""
    session = get_user_session(user_id)
    
    # Guardar el producto de interés
    session['data']['product_interest'] = text
    session['state'] = 'completed'
    
    # Notificar al asesor
    notify_advisor_general(user_id, text)
    
    message = "Perfecto, he registrado tu interés. Christian te contactará para brindarte más información."
    send_whatsapp_message(user_id, message)
    logger.info(f"Usuario {user_id} interesado en: {text}")

def send_imss_initial_message(user_id):
    """Envía mensaje inicial con información del préstamo IMSS"""
    message = """👋 ¡Perfecto! Te ayudo con tu préstamo IMSS.
...
Para comenzar, dime tu *pensión mensual aproximada* (ej. 7500)."""
    ...
    logger.info(f"Usuario {user_id} en estado IMSS: ask_pension")
...
def handle_pension_response(user_id, text):
    ...
    # Caso especial: dos números en el primer mensaje
    if len(amounts) >= 2:
        pension = min(amounts)
...
def handle_nomina_response(user_id, text):
    ...
    if text_lower in positive_responses:
        ...
        send_whatsapp_message(user_id, message)
        notify_advisor_imss(user_id, True)
        ...
        send_whatsapp_message(user_id, message)
        notify_advisor_imss(user_id, False)
...
def notify_advisor_imss(user_id, accepts_nomina):
    """Envía notificación al asesor para IMSS"""
    session = get_user_session(user_id)
...
🏦 Nómina: NO"""
    ...
    logger.info(f"Notificación IMSS enviada al asesor para {user_id}")

def notify_advisor_general(user_id, product_interest):
    """Envía notificación al asesor para consulta general"""
    message = f"""🔔 NUEVA CONSULTA
📞 {user_id}
💬 {product_interest}"""
    
    send_whatsapp_message(ADVISOR_WHATSAPP, message)
    logger.info(f"Notificación general enviada al asesor para {user_id}")

def handle_user_message(user_id, message_text):
    """Maneja el mensaje del usuario según su estado actual"""
    session = get_user_session(user_id)
    
    logger.info(f"Procesando mensaje para {user_id} en estado: {session['state']}")
    
    if session['state'] == 'init':
        send_welcome_message(user_id)
        
    elif session['state'] == 'ask_imss_interest':
        handle_imss_interest(user_id, message_text)
        
    elif session['state'] == 'ask_product':
        handle_product_interest(user_id, message_text)
        
    elif session['campaign'] == 'imss':
        # Flujo IMSS
        if session['state'] == 'ask_pension':
            ...
            # Si ya completó el flujo, reiniciar
            send_welcome_message(user_id)
            ...
            for change in changes:
                ...
                for message in messages:
                    if message.get('type') == 'text':
                        ...
                        # Manejar el mensaje según el estado del usuario
